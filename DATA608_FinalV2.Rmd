...
* [NYC Department of Environment] (http://a816-dohbesp.nyc.gov/IndicatorPublic/VisualizationData.aspx?id=2063,4466a0,113,Summarize)
...
```{r warning=FALSE, message=FALSE}
incomec<-sqldf("select * from incomeb order by income asc limit 5")
nationalc<-sqldf("select * from nationalb order by obesity desc limit 5")
...
ggplot(incomec, aes(x=reorder(state,income),round(income, digits=2)))+ 
  ...
  ggtitle("Top 5 States by Median Household Income");
...
ggplot(nationalc, aes(x=reorder(state,Obesity),round(Obesity, digits=2)))+ 
  ...
  ggtitle("Top 5 States by Percent of Adults Classified as Obese");
...
As a case study, we can focus on a single state to see an even more granular look at the data. As a proof of concept, we will look into New York which happens to be my state. 
